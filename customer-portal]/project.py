...
def main():
    """Main function to launch the Gradio interface with a logo."""
    
    # Load the available PDFs from GCS
    pdf_files = list_pdfs()

    with gr.Blocks() as iface:
        # Add the logo at the top (Ensure `download.png` is in the same directory)
        gr.Image("download.png", label="LLoyds banking group", width=100, height=100)

        # Page Title & Description
        gr.Markdown("# 📄 Customer Support Portal")
        gr.Markdown("Select a PDF to extract important information.")

        # Inputs
        with gr.Row():
            pdf_dropdown = gr.Dropdown(choices=pdf_files, label="Select the document")
            query_box = gr.Textbox(label="Enter your question")

        # Outputs
        answer_box = gr.Textbox(label="Answer")
        log_box = gr.Textbox(label="Processing Log")

        # Submit Button
        submit_btn = gr.Button("Submit")
        submit_btn.click(gradio_interface, inputs=[pdf_dropdown, query_box], outputs=[answer_box, log_box])
